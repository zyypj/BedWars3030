package com.tomkeuper.bedwars.support.version.v1_21_R7;
...
public class DefaultGenAnimation implements IGeneratorAnimation {
    ...
    public void run() {
        ...
        final var positionMoveRotation = new PositionMoveRotation(armorStand.dJ(), delta, 0, 0);
        final Set<Relative> set = new HashSet<>();

        PacketPlayOutEntityTeleport teleportPacket = new PacketPlayOutEntityTeleport(armorStand.aA(), positionMoveRotation, set, false);

        PacketPlayOutEntity.PacketPlayOutRelEntityMoveLook moveLookPacket = new PacketPlayOutEntity.PacketPlayOutRelEntityMoveLook(armorStand.aA(), (short) 0, (short) ((getArmorStandMotY() - lastMotY)*128), (short) 0, (byte) getArmorStandYAW(), (byte) 0, false);

        for (Player p : loc.getWorld().getPlayers()) {
            v1_21_R7.sendPackets(p, teleportPacket, moveLookPacket);
        }
        tickCount++;
    }
    ...
    private void setArmorStandMotY(double y) {
        armorStand.k(new Vec3D(0, y, 0));
    }

    private void addArmorStandMotY(double y) {
        armorStand.k(new Vec3D(0, getArmorStandMotY() + y, 0));
    }

    private double getArmorStandMotY() {
        return armorStand.dN().h;
    }
}
